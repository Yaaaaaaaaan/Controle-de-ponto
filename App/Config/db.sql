...
create table fotos(
    ...
    constraint fotos_ibfk_1 foreign key (id_usuario) references usuarios (id_usuario) on delete cascade,
    constraint fotos_ibfk_2 foreign key (album_id) references albuns (album_id) on delete cascade
);
...
create table historicos_acoes(
    historico_id bigint unsigned auto_increment primary key,
    id_usuario bigint unsigned not null,
    descricao text null,
...
create table registros_ponto(
    registro_id bigint unsigned auto_increment primary key,
    id_usuario bigint unsigned not null,
    data_registro date not null,
    status varchar(50) default 'Verificação pendente' null,
    observacao text null,
    constraint idx_usuario_data_unica unique (id_usuario, data_registro),
...
create table tokens_autenticacao(
    ...
    constraint idx_id_usuario_unico unique (id_usuario),
    constraint tokens_autenticacao_ibfk_1 foreign key (id_usuario) references usuarios (id_usuario) on delete cascade
);